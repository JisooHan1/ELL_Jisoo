...
class MDS:
    ...
    def get_cls_covariances(self, class_features):
        ...
        self.cls_covariances = torch.einsum('ni,nj->ij', deviations, deviations) / (N-1)  # (channel, channel)
    ...
    def get_mds_scores(self, test_dataloader, cls_means, cls_covariances, epsilon=0.001):
        ...
        cls_means = [mean.to(device) for mean in cls_means]
        cls_covariances = cls_covariances.to(device)
        ...
        for inputs, _ in test_dataloader:
            ...
            confidence_scores = mahalanobis_distances[torch.arange(batch_size), c_hat]  # (batch,)
