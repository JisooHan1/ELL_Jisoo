...
class ReActDetector:
    ...
    def calculate_threshold(self, samples, quantile=0.95):
        ...
        print(f"Input samples range: [{samples.min().item():.4f}, {samples.max().item():.4f}]")
    ...
    def react(self, dataloader):
        ...
        for inputs, _ in dataloader:
            ...
            all_penultimate_values.append(penultimate.detach().cpu())
        ...
        # 테스트 데이터의 분포 출력
        all_penultimate = torch.cat(all_penultimate_values, dim=0).view(-1)  # (num_samples,)
        print(f"\nTest data statistics:")
        print(f"Penultimate values range: [{all_penultimate.min().item():.4f}, {all_penultimate.max().item():.4f}]")
        print(f"Values > threshold: {(all_penultimate > self.c).float().mean().item()*100:.2f}%")
        print(f"Mean: {all_penultimate.mean().item():.4f}")
        print(f"Std: {all_penultimate.std().item():.4f}")
