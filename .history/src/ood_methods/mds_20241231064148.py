...
class MDS(BaseOOD):
    ...
    def get_class_features(self, id_dataloader):
        for inputs, labels in id_dataloader:
            inputs, labels = inputs.to(device), labels.to(device)  #  (batch)
    ...
    def get_covariance(self, class_features):
        ...
        cls_covariances = []
        for cls in range(self.num_classes):
            cls_deviations = class_data[cls] - self.id_cls_means  # (sample x channel)
            cls_covariances.append(torch.einsum('si, sj -> ij', cls_deviations, cls_deviations))  # (channel x channel)

        self.id_covariances = torch.stack(cls_covariances).sum(dim=0) / N
    ...
    def ood_score(self, inputs):
        ...
        id_cls_covariances = self.id_covariances  # (channel x channel)
